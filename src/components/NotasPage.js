...
const NotasPage = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        console.log("Notes fetched:", response.data); // Verifica los datos recibidos
    ...
    if (id_escena) {
      console.log("Fetching notes for scene ID:", id_escena); // Confirma que el ID está presente
      fetchNotes();
    } else {
      setErrorMessage("Scene ID is not provided.");
    }
  }, [id_escena]);
  ...
  const handleAddNote = async () => {
    if (noteText.trim().length > 0) {
      ...
      setErrorMessage('Note text is empty.');
    }
  };
          ...
          {notes.length > 0 ? (
            notes.map((note) => (
              <div className='note' key={note.id_nota}>
                <textarea rows='3' cols='30' value={note.contenido}></textarea>
