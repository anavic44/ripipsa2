...
const NotasPage = () => {
  ...
  const { id_escena } = useParams(); // Cambiar id_escena por id_objeto
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const response = await axios.get(`http://localhost:2023/api/notas/${id_escena}`); // Cambiar id_escena por id_objeto
    ...
    fetchNotes();
  }, [id_escena]); // Cambiar id_escena por id_objeto
